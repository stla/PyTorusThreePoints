...
alpha_ = np.linspace(0, 2*pi, 121)[:120]
...
for i, triplet in enumerate(triplets):
    ...
    meshes[i] = mesh0.transform(matAndRad["matrix"])
...
for i, matrix in enumerate(matrices):
    ...
    for light in pltr.renderer.lights:
        ...
        #light.position = rmatrix(-3)[0:3, 0:3].dot(np.asarray(light.position))
    for j in range(10):
        pltr.add_mesh(
            meshes[j], smooth_shading=True, cmap=["#440154", "#FDE725"], 
            specular=10, show_scalar_bar=False
        )
    png = "zpic_%03d.png" % i
